<template>
	<div v-loading="loading" element-loading-text="获取数据中">
		<div class="handle-box">
			<div class="btn">
				<el-input placeholder="请输入用户ID" v-model="user_id" class="search" @keyup.enter.native="search(user_id)">
					<el-button slot="append" icon="el-icon-search" @click="search(user_id)"></el-button>
				</el-input>
			</div>
		</div>
		...
		<el-table :data="tableData" border :header-cell-style="{background:'#f0f0f0'}" max-height="620">
			...
			<el-table-column prop="avatarUrl" label="头像">
				<template slot-scope="scope">
					<img :src="scope.row.avatarUrl" style="max-width:180px;max-height:80px;" />
				</template>
			</el-table-column>
			...
			<el-table-column label="操作" width="300px">
				<template slot-scope="scope">
					<el-button size="mini" type="primary" @click="handleOpenPermission(scope.$index, scope.row)">开通权限</el-button>
					<el-button size="mini" type="primary" @click="handleBanUsePermission(scope.$index, scope.row)">禁止使用权限</el-button>
				</template>
			</el-table-column>
		</el-table>

		<el-dialog title="开通权限" :visible.sync="dialogOpenPermission" :close-on-click-modal="false" width="500px" @close="closePermissionForm">
			<div class="box">
				<el-form :model="openPermissionForm">
					<div v-for="(item,index) in openPermissionForm.service" :key="index">
						<el-form-item label="选择学校">
							<el-select v-model="item.school_id" placeholder="请选择学校" @change="schoolChange">
						...
						<el-form-item label="选择商品">
							<el-select v-model="item.product_id" placeholder="选择商品">
								<el-option v-for="item in products" :label="item.title" :value="item.id" :key="item.index"></el-option>
							</el-select>
						</el-form-item>
						<el-form-item label="选择学生">
							<el-select v-model="item.student_id" placeholder="选择学生">
								<el-option v-for="item in students" :label="item.name" :value="item.id" :key="item.index"></el-option>
							</el-select>
						</el-form-item>
					</div>
					<el-form-item label="操作">
						<el-button type="success" @click="addopenPermissionForm">添加</el-button>
						<el-button type="success" @click="delopenPermissionForm">删除</el-button>
					</el-form-item>
					<div class="submit">
						<el-form-item>
		...
		<el-dialog title="禁止使用权限" :visible.sync="dialogBanUsePermission" :close-on-click-modal="false">
			<div class="box">
				<el-form>
					<el-form-item label="选择禁用的学校">
						<el-transfer v-model="banSchoolList" :data="schoolList" :titles="['选择学校', '禁用学校']" :button-texts="['取消', '禁用']"
						 @left-check-change="changeLeft" @right-check-change="changeRight" @change="newBanUsePermission"></el-transfer>
					</el-form-item>
					<div class="submit">
						<!-- <el-form-item>
              <el-button type="primary" @click="newBanUsePermission">提交</el-button>
            </el-form-item>-->
					</div>
				</el-form>
				<!-- <el-tabs v-model="activeName">
				<el-tab-pane label="禁用学校" name="school">
					<el-form :model="BanUsePermissionForm">
						<el-form-item label="选择禁用的学校">
						</el-form-item>
					</el-form>
				</el-tab-pane>
				<el-tab-pane label="禁用开启权限" name="permission">
				</el-tab-pane>
        </el-tabs>-->
			</div>
		</el-dialog>
		...
		<div class="block">
			<el-pagination @current-change="currentChange" :current-page.sync="current" :page-size="size" layout="sizes, prev, pager, next, jumper"
...
<script>
	...
	export default {
		...
		methods: {
			...
			// getSchool() {
			//   var self = this;
			//   API.banProducts(self.banUsePermissionForm.user_id).then(res => {
			//     res.forEach(item => {
