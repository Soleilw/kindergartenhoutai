    ...
    <el-table :data="tableData" border :header-cell-style="{ background: '#f0f0f0' }" max-height="620">
      ...
      <el-table-column label="操作" width="300px">
        <template slot-scope="scope">
          ...
          <el-button size="mini" type="primary" @click="handleBanUsePermission(scope.$index, scope.row)">禁止使用权限
          </el-button>
        </template>
      </el-table-column>
    </el-table>

    <el-dialog title="开通权限" :visible.sync="dialogOpenPermission" :close-on-click-modal="false" width="500px"
      ...
      <div class="box">
        <el-form>
          <el-form-item label="选择禁用的学校">
            <el-transfer v-model="banSchoolList" :data="schoolList" :titles="['选择学校', '禁用学校']"
              :button-texts="['取消', '禁用']" @left-check-change="changeLeft" @right-check-change="changeRight"
              @change="newBanUsePermission"></el-transfer>
          </el-form-item>
        </el-form>
      </div>
    </el-dialog>
