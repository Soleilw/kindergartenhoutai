					...
					<div v-for="(item,index) in form.grades" :key="index">
						<el-form-item label="年级选择">
							<el-select v-model="item.title" placeholder="请选择年级" @change="gradeChange">
								<el-option v-for="selectItem in grade" :key="selectItem.value" :label="selectItem.label" :value="selectItem.value"></el-option>
							</el-select>
						</el-form-item>
				...
				<template slot-scope="scope">
					<el-dropdown>
						...
						<el-dropdown-menu slot="dropdown">
							...
							<el-dropdown-item>
								<el-button size="mini" type="primary" @click="handleShowFinancial(scope.$index, scope.row)">查看财务</el-button>
							</el-dropdown-item>
							<el-dropdown-item>
		...
		<el-dialog title="查看学校所有" :visible.sync="dialogUser" width="80%" @close="closeShowUser">
			<div class="box">
				...
				<template v-if="user == '用户'">
					...
					<div class="block">
						<el-pagination @current-change="handleUserChange" :current-page.sync="currentUserPage" :page-sizes="[10, 20, 30, 40, 50]"
						 :page-size="userPageSize" layout="sizes, prev, pager, next, jumper" :total="userTotalPage" @size-change="handleUserSizeChange"></el-pagination>
					</div>
				</template>
				<template v-if="user == '学生'">
					...
					<div class="block">
						<el-pagination @current-change="handleStudentChange" :current-page.sync="currentStudentPage" :page-sizes="[10, 20, 30, 40, 50]"
						 :page-size="studentPageSize" layout="sizes, prev, pager, next, jumper" :total="studentTotalPage" @size-change="handleStudentSizeChange"></el-pagination>
					</div>
				</template>
				<template v-if="user == '教师'">
					<el-table :data="teacherData" border :header-cell-style="{background:'#f0f0f0'}">
						...
						<el-table-column prop="grades.title" label="年级">
							<template slot-scope="scope">
								<span v-text="scope.row.grades.title == null ? '无' : scope.row.grades.title "></span>
							</template>
						</el-table-column>
						<el-table-column prop="class.title" label="班级">
							<template slot-scope="scope">
								<span v-text="scope.row.class.title == null ? '无' : scope.row.class.title "></span>
							</template>
						</el-table-column>
					...
					<div class="block">
						<el-pagination @current-change="handleTeacherChange" :current-page.sync="currentTeacherPage" :page-sizes="[10, 20, 30, 40, 50]"
						 :page-size="teacherPageSize" layout="sizes, prev, pager, next, jumper" :total="teacherTotalPage" @size-change="handleTeacherSizeChange"></el-pagination>
					</div>
				</template>
				...
				<template v-if="user == '访客'">
					...
					<div class="block">
						<el-pagination @current-change="handleVisitorChange" :current-page.sync="currentVisitorPage" :page-sizes="[10, 20, 30, 40, 50]"
						 :page-size="visitorPageSize" layout="sizes, prev, pager, next, jumper" :total="visitorTotalPage" @size-change="handleVisitorSizeChange"></el-pagination>
					</div>
				</template>
			</div>
		</el-dialog>
			...
			<div class="box">
				<div class="btn">
					<el-select v-model="financial" @change="handleFinancial" placeholder="请选择">
						<el-option v-for="item in financialList" :key="item.value" :label="item.label" :value="item.value"></el-option>
					</el-select>
				</div>
				...
				<div v-if="financial === '财务统计'">
					...
					<div class="block">
						<el-pagination @current-change="handleFinanceChange" :current-page.sync="currentFinancePage" :page-sizes="[10, 20, 30, 40, 50]"
						 :page-size="financePageSize" layout="sizes, prev, pager, next, jumper" :total="financeTotalPage" @size-change="handleFinanceSizeChange"></el-pagination>
					</div>
				</div>
			</div>
		</el-dialog>
